...
static PAYLOAD_TYPE_E gs_enPayloadType = PT_ADPCMA;//PT_LPCM;//PT_ADPCMA;
...
******************************************************************************/
HI_S32 SAMPLE_AUDIO_AdecAo(AIO_ATTR_S *pstAioAttr, ADEC_CHN  AdChn)
{
    HI_S32      s32Ret;
...
AIO_ATTR_S gs_AioAttr;
AUDIO_RESAMPLE_ATTR_S gs_AiReSampleAttr;
AUDIO_RESAMPLE_ATTR_S gs_AoReSampleAttr;
AIO_ATTR_S gs_HdmiAoAttr;

/***************************************************************
*名称: void audio_variableinit(VB_CONF_S *p, HI_U32 u32ViChnCnt, PIC_SIZE_E enCodeSize)
...
{  
    ...
    gs_AioAttr.enSamplerate = AUDIO_SAMPLE_RATE_8000;/* 8K的采样速率 */
    gs_AioAttr.enBitwidth = AUDIO_BIT_WIDTH_16;/* 16位 */
    gs_AioAttr.enWorkmode = AIO_MODE_I2S_SLAVE;
    gs_AioAttr.enSoundmode = AUDIO_SOUND_MODE_MONO;/* 单端 */
    gs_AioAttr.u32EXFlag = 1;
    gs_AioAttr.u32FrmNum = 30;
    gs_AioAttr.u32PtNumPerFrm = SAMPLE_AUDIO_PTNUMPERFRM;
    gs_AioAttr.u32ChnCnt = 8;/* 5路音频同时录音 */
    gs_AioAttr.u32ClkSel = 0;

    /* config ao resample attr if needed */
    if (HI_TRUE == gs_bAioReSample)
    {
        gs_AioAttr.enSamplerate = AUDIO_SAMPLE_RATE_32000;
        gs_AioAttr.u32PtNumPerFrm = SAMPLE_AUDIO_PTNUMPERFRM * 4;

        /* ai 32k -> 8k */
        gs_AiReSampleAttr.u32InPointNum = SAMPLE_AUDIO_PTNUMPERFRM * 4;
        gs_AiReSampleAttr.enInSampleRate = AUDIO_SAMPLE_RATE_32000;
        gs_AiReSampleAttr.enReSampleType = AUDIO_RESAMPLE_4X1;
        gs_pstAiReSmpAttr = &gs_AiReSampleAttr;

        /* ao 8k -> 32k */
        gs_AoReSampleAttr.u32InPointNum = SAMPLE_AUDIO_PTNUMPERFRM;
        gs_AoReSampleAttr.enInSampleRate = AUDIO_SAMPLE_RATE_8000;
        gs_AoReSampleAttr.enReSampleType = AUDIO_RESAMPLE_1X4;
        gs_pstAoReSmpAttr = &gs_AoReSampleAttr;
    }
    else
    {
...
HI_U32 audio_init(AIO_ATTR_S *pstAioAttr)
{
    HI_S32 s32Ret = HI_SUCCESS;
    VB_CONF_S stVbConf;

    memset(&stVbConf, 0x00, sizeof(VB_CONF_S));
...
HI_S32 main_audio(int argc, char *argv[])
{
    ...
    while ((ch = getchar()) != 'q')
    {
        switch (ch)
        {
            case '1':
            {
                s32Ret = SAMPLE_AUDIO_AiAo(&gs_AioAttr);/* AI to AO*/
                break;
            }
            case '2':
            {
                s32Ret = SAMPLE_AUDIO_AiAenc(&gs_AioAttr);/* send audio frame to AENC channel form AI, save them*/
                break;
            }
            case '3':
            {
                s32Ret = SAMPLE_AUDIO_AdecAo(&gs_AioAttr, 0);/* read audio stream from file,decode and send AO*/
                break;
            }
            case '4':
            {
                s32Ret = SAMPLE_AUDIO_AdecAo(&gs_AioAttr, 1);/* read audio stream from file,decode and send AO*/
                break;
            }
            case '5':
            {
                s32Ret = SAMPLE_AUDIO_AdecAo(&gs_AioAttr, 2);/* read audio stream from file,decode and send AO*/
                break;
            }
            case '6':
            {
                s32Ret = SAMPLE_AUDIO_AdecAo(&gs_AioAttr, 3);/* read audio stream from file,decode and send AO*/
                break;
            }
            case '7':
            {
                s32Ret = SAMPLE_AUDIO_AdecAo(&gs_AioAttr, 4);/* read audio stream from file,decode and send AO*/
                break;
            }
            case '8':
            {
                s32Ret = SAMPLE_AUDIO_AdecAo(&gs_AioAttr, 5);/* read audio stream from file,decode and send AO*/
                break;
            }
            case '9':
            {
                s32Ret = SAMPLE_AUDIO_AdecAo(&gs_AioAttr, 6);/* read audio stream from file,decode and send AO*/
                break;
            }
            case 'a':
            {
                s32Ret = SAMPLE_AUDIO_AdecAo(&gs_AioAttr, 7);/* read audio stream from file,decode and send AO*/
                break;
            }
            case 'b':
            {
                ...
                gs_AoReSampleAttr.u32InPointNum = SAMPLE_AUDIO_PTNUMPERFRM;
                gs_AoReSampleAttr.enInSampleRate = AUDIO_SAMPLE_RATE_8000;
                gs_AoReSampleAttr.enReSampleType = AUDIO_RESAMPLE_1X6;
                gs_pstAoReSmpAttr = &gs_AoReSampleAttr;
                
                memcpy(&gs_HdmiAoAttr, &gs_AioAttr, sizeof(AIO_ATTR_S));
                gs_HdmiAoAttr.enBitwidth = AUDIO_BIT_WIDTH_16;
                gs_HdmiAoAttr.enSamplerate = AUDIO_SAMPLE_RATE_48000;
                gs_HdmiAoAttr.u32PtNumPerFrm = gs_AioAttr.u32PtNumPerFrm * 6;
                gs_HdmiAoAttr.enWorkmode = AIO_MODE_I2S_MASTER;
                gs_HdmiAoAttr.u32ChnCnt = 2;
                s32Ret = SAMPLE_AUDIO_AiHdmiAo(&gs_AioAttr, &gs_HdmiAoAttr);/* AI to AO(Hdmi)*/
                gs_pstAoReSmpAttr = NULL;
                break;
            }
