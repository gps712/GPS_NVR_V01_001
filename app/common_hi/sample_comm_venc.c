...
static pthread_t gs_VencPid_snap;
static SAMPLE_VENC_GETSTREAM_PARA_S gs_stPara;
static SAMPLE_VENC_GETSTREAM_PARA_S gs_stPara_snap;
...
/******************************************************************************
* function : venc bind vpss      tkdsltsltss      
******************************************************************************/
HI_S32 SAMPLE_COMM_VENC_BindVI(VENC_GRP GrpChn,HI_S32 VIGrp,HI_S32 VIChn)
{
    HI_S32 s32Ret = HI_SUCCESS;
    MPP_CHN_S stSrcChn;
    MPP_CHN_S stDestChn;
...
HI_S32 SAMPLE_COMM_VENC_SaveH264(FILE* fpH264File, VENC_STREAM_S *pstStream)
{
    ...
    printf(" LEN=%5d HEX:",len);
	len = pstStream->pstPack[0].u32Len[0];
	if(pstStream->pstPack[0].u32Len[0] < 20)
		len = pstStream->pstPack[0].u32Len[0];
	else
		len = 20;
	//printf_hex_data(pstStream->pstPack[0].pu8Addr[0],len);
...
* funciton : snap process
******************************************************************************/
HI_S32 SAMPLE_COMM_VENC_SnapProcess2(VENC_GRP VencGrp, VENC_CHN VencChn, VPSS_GRP VpssGrp, VPSS_CHN VpssChn)
{
    struct timeval TimeoutVal;
    fd_set read_fds;
    HI_S32 s32VencFd;
    ...
    ******************************************/
    s32Ret = SAMPLE_COMM_VENC_BindVpss(VencGrp, VpssGrp, VpssChn);
    //s32Ret = SAMPLE_COMM_VENC_BindVI(VencGrp, VpssGrp/4, VpssGrp);
        ...
        else
        {
            for (i = 0; i < s32ChnTotal; i++)
            {
                if (FD_ISSET(VencFd[i], &read_fds))
                {
                    ...
                    *******************************************************/
                    stStream.pstPack = (VENC_PACK_S*)malloc(sizeof(VENC_PACK_S) * stStat.u32CurPacks);
					printf("\nstlen=%6d,p=%d  ",sizeof(VENC_PACK_S) * stStat.u32CurPacks,stStat.u32CurPacks);
                    if (NULL == stStream.pstPack)
                    {
...
* funciton : get stream from each channels and save them
******************************************************************************/
HI_VOID* SAMPLE_COMM_VENC_GetVencSnapProc(HI_VOID *p)
{
    HI_S32 i;
    HI_S32 s32ChnTotal;
    VENC_CHN_ATTR_S stVencChnAttr;
	...
	pstPara = (SAMPLE_VENC_GETSTREAM_PARA_S*)p;
	s32ChnTotal = pstPara->s32Cnt;
	if(HI_TRUE == pstPara->bThreadStart)
	{
	    for (i = 0; i < s32ChnTotal; i++)
	    {
	        /*** main frame **/
			VpssGrp = i;
			s32Ret = SAMPLE_COMM_VENC_SnapProcess2(2*s32ChnTotal, 2*s32ChnTotal, VpssGrp, VPSS_BSTR_CHN);
			if (HI_SUCCESS != s32Ret)
			{
...
HI_S32 SAMPLE_COMM_VENC_StartGetStream(HI_S32 s32Cnt)
{
    ...
    gs_stPara_snap.bThreadStart = HI_TRUE;
    gs_stPara_snap.s32Cnt = s32Cnt/2;
    s32Ret = pthread_create(&gs_VencPid, 0, SAMPLE_COMM_VENC_GetVencStreamProc, (HI_VOID*)&gs_stPara);

	
	return s32Ret+pthread_create(&gs_VencPid_snap, 0, SAMPLE_COMM_VENC_GetVencSnapProc, (HI_VOID*)&gs_stPara_snap);
}
...
HI_S32 SAMPLE_COMM_VENC_StopGetStream()
{
	...
	if (HI_TRUE == gs_stPara_snap.bThreadStart)
    {
        gs_stPara_snap.bThreadStart = HI_FALSE;
        pthread_join(gs_VencPid_snap, 0);
    }
    return HI_SUCCESS;
}
